...
//			{
//				AlarmWatchdogState &= ~0x02; //���Ź� ʹ�� �ر�  1 ��
//			}
//			else
//			{
...
//			//CanTransmit(HM7280_IOT_ID_ALARM_WDG,(uint8_t*)&g_Hm7280IotAlarmWdg,8);// �����Լ����Ź�״̬
//			can_message_transmit(CAN0, &g_can0TxMessage_bms);
//			AlarmWatchdogState_blk =  AlarmWatchdogState;
//		}
	
	
	if(pag_watchwdg == 0) // �·�����PPID
	{
		g_can0TxMessage_bms.tx_sfid = 0x00;
	...
	if(HAL_GetTick()-g_CanTransmitState.t5000ms>=5000) // ����CAN����
    {
		...
		if(g_Hm7280SerialDirty == FALSE)
		{
			...
			if(pag_watchwdg == 0) 
			{
				if((PaygGetFreeState())) //free
				{
					g_can0TxMessage_bms.tx_data[2] = 0x00; //���Ź� ʹ�� �ر�  1 ��
				}
				else
				{
			...
			can_message_transmit(CAN0, &g_can0TxMessage_bms); // �����Լ�ι��
		}
		...
		CanHm7280BuildSerialPayload(); // ��ȡ������PPID
	...
	if (g_CcsEnergyLimittime > 0 &&  ((HAL_GetTick() - g_CcsEnergyLimittime) > (g_CcsEnergyLimittime_count)))  // ����ʱ����� ����Ϊ��λ- g_UserSet.time_blk
	{
